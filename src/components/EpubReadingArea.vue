...
<script setup lang="ts">
  ...
  const loadChapters = async (file: TxtFile) => {
    try {
      loading.value = true;
      const parsedChapters = await FileService.loadEpubFile(file.path);
      emit('chapters-loaded', parsedChapters);
      message.success(`已加载 ${parsedChapters.length} 个章节`);

      const chapterToLoad = props.progress?.current_chapter !== undefined ? props.progress.current_chapter : 0;
      emit('chapter-changed', chapterToLoad);

      await nextTick();
      const scrollPos = props.progress?.scroll_position;
      if (contentRef.value && scrollPos > 0) {
        setTimeout(() => {
          if (contentRef.value) {
